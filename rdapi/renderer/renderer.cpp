...
void RDRenderer_Mnemonic(RDRenderer* r, const char* s, rd_type theme) { CPTR(Renderer, r)->renderMnemonic(s ? s : "mnemonic???", theme); }
void RDRenderer_MnemonicWord(RDRenderer* r, const char* s, rd_type theme) { CPTR(Renderer, r)->renderMnemonicWord(s ? s : "mnemonic???", theme); }
void RDRenderer_Register(RDRenderer* r, const char* s) { CPTR(Renderer, r)->renderRegister(s ? s : "reg???"); }
void RDRenderer_Constant(RDRenderer* r, const char* s) { CPTR(Renderer, r)->renderConstant(s ? s : "cnst???"); }
void RDRenderer_Text(RDRenderer* r, const char* s) { CPTR(Renderer, r)->renderText(s ? s : "text???"); }
void RDRenderer_Themed(RDRenderer* r, const char* s, rd_type theme) { CPTR(Renderer, r)->renderText(s ? s : "text???", theme); }
