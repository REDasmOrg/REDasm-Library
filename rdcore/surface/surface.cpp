#include "surface.h"
#include "../document/model/iterators/blocksiterator.h"
#include "../document/document.h"
#include "../context.h"
#include "renderer.h"
#include <cmath>

Surface::Surface(Context* ctx, rd_flag flags, uintptr_t userdata): SurfaceRenderer(ctx, flags), m_cursor(this, ctx), m_path(this), m_userdata(userdata)
{
...
const Cursor* Surface::cursor() const { return &m_cursor; }
Cursor* Surface::cursor() { return &m_cursor; }
size_t Surface::getPath(const RDPathItem** path) const { return m_path.getPath(path); }
rd_address Surface::currentAddress() const { return m_cursor.currentAddress(); }
...
void Surface::scroll(int nx, int ny)
{
    int oldfirstcol = m_firstcol;
    m_firstcol = std::max(m_firstcol + nx, 0);

    if(ny) this->scrollAddress(this->firstAddress(), ny);
    ...
    if((m_range.first != RD_NVAL) || (m_firstcol != oldfirstcol))
    {
        ...
        if(m_cursor.currentRow() != -1 && m_cursor.currentRow() < static_cast<int>(m_rows.size()))
            m_cursor.setCurrentAddress(m_rows.at(m_cursor.currentRow()).address);
...
void Surface::scrollAddress(rd_address address, int n)
{
    if(!n) return;
    ...
    {
        BlocksIterator it(addressspace, address);
        std::optional<RDBlock> b;

        if(n > 0) // Forward scroll
        {
...
void Surface::checkSelection()
{
    ...
    RDSurfacePos startsel = m_cursor.startSelection(), endsel = m_cursor.endSelection();
