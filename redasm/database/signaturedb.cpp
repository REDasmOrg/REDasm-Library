...
std::string SignatureDB::assembler() const { PIMPL_P(const SignatureDB); return p->assembler(); }
std::string SignatureDB::name() const { PIMPL_P(const SignatureDB); return p->name();  }
...
void SignatureDB::setAssembler(const std::string &assembler) { PIMPL_P(SignatureDB); p->setAssembler(assembler); }
void SignatureDB::setName(const std::string &name) { PIMPL_P(SignatureDB); p->setName(name); }
bool SignatureDB::load(const std::string &sigfilename) { PIMPL_P(SignatureDB); return p->load(sigfilename); }
bool SignatureDB::save(const std::string &sigfilename) { PIMPL_P(SignatureDB); return p->save(sigfilename); }
